...
struct GammaGraph {
    ...
    void turnOnNode(GammaNode * node) {
        if (this->transitions.empty() or node->correct)
            return;
        node->correct = true;
        for (auto & transition: this->transitions) {
            if (transition.first == node) {
                turnOnNode(transition.second);
            }
            if (transition.second == node) {
                turnOnNode(transition.first);
            }
        }
    }
